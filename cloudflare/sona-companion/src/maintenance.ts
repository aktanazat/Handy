...
async function markDone(env: Env, job: MaintenanceJobRow, now: number): Promise<void> {
...
async function retryLater(
  ...
  await env.DB.prepare(
    ...
    .bind(progress, now + JOB_RETRY_MS, errorCode, now, job.job_id, job.lease_token)
    .run();
}
...
async function releaseUploadUsage(
  ...
  await env.DB.batch([
    ...
    env.DB
      ...
      .bind(session.total_bytes, session.total_bytes, session.vault_id, job.job_id, job.lease_token),
  ]);
}
...
async function cleanUpload(
  ...
  await Promise.all(
    chunks.map((chunk) => env.CIPHERTEXT.delete(r2ChunkKey(job.vault_id, session.upload_id, chunk.index))),
  );
  if (end < plan.length) {
...
async function cleanObject(
  ...
  const candidates = await env.DB.prepare(
    ...
    .bind(job.vault_id, job.target_id, job.vault_id, job.target_id, MAINTENANCE_PAGE_SIZE)
    .all<StoredChunkRow>();
  await Promise.all(candidates.results.map((chunk) => env.CIPHERTEXT.delete(chunk.r2_key)));
    ...
    for (const chunk of candidates.results) {
      statements.push(
        ...
        env.DB
          ...
          .bind(chunk.byte_size, chunk.byte_size, job.vault_id, job.job_id, job.lease_token),
      );
    }
    statements.push(
      env.DB
...
async function reconcileUpload(
  ...
  const acceptedIndexes = new Set(accepted.results.map((row) => row.chunk_index));
  ...
  while (inspected < MAINTENANCE_PAGE_SIZE && inspected < plan.length) {
    ...
    if (!acceptedIndexes.has(chunk.index)) {
      ...
      if (stored !== null) {
        if (stored.size !== chunk.size || stored.customMetadata?.sha256 !== chunk.sha256) {
          throw problem("integrity_failed");
        }
        await env.DB.prepare(
...
async function compactChanges(env: Env, job: MaintenanceJobRow, now: number): Promise<void> {
  ...
  await env.DB.batch([
    env.DB
      ...
      .bind(job.vault_id, cutoff, MAINTENANCE_PAGE_SIZE, job.job_id, job.lease_token),
    env.DB
      .prepare(
...
async function processClaimedJob(env: Env, job: MaintenanceJobRow, now: number): Promise<void> {
...
export async function processMaintenanceJob(env: Env, jobId: string, now = Date.now()): Promise<void> {
  ...
  try {
    ...
    const code = error instanceof ApiProblem ? error.code : "dependency_unavailable";
    await retryLater(env, job, now, code);
    logMaintenance(env, job.job_class, 503, code);
  }
}

async function processDueJobsInOrder(env: Env, jobIds: readonly string[], now: number, index = 0): Promise<void> {
  ...
  for (const session of expired.results) {
    ...
    statements.push(
      ...
      env.DB
        ...
        .bind(jobId, session.vault_id, session.upload_id, now, now, now, session.upload_id),
    );
  }
...
async function expireShares(env: Env, now: number): Promise<void> {
  const expired = await env.DB.prepare(
    ...
    .bind(now, MAINTENANCE_PAGE_SIZE)
    .all<{ share_id: string; state: "pending" | "active"; upload_id: string; vault_id: string }>();
  const statements: D1PreparedStatement[] = [];
  for (const share of expired.results) {
    ...
    if (share.state === "pending") {
      statements.push(
        env.DB
          .prepare("UPDATE shares SET state = 'expired' WHERE vault_id = ? AND share_id = ? AND state = 'pending'")
          .bind(share.vault_id, share.share_id),
        env.DB
          .prepare("UPDATE upload_sessions SET state = 'expired', updated_at = ? WHERE upload_id = ? AND state = 'active'")
    ...
    statements.push(
      env.DB
        .prepare("UPDATE shares SET state = 'expired' WHERE vault_id = ? AND share_id = ? AND state = 'active'")
        .bind(share.vault_id, share.share_id),
      env.DB
        .prepare(
          "INSERT INTO maintenance_jobs (job_id, vault_id, job_class, target_id, payload_json, state, next_attempt_at, created_at, updated_at) VALUES (?, ?, 'cleanup_share', ?, '{}', 'queued', ?, ?, ?)",
...
async function purgeExpiredMutationNonces(env: Env, now: number): Promise<void> {
  await env.DB.prepare(
...
async function scheduleChangeCompaction(env: Env, now: number): Promise<void> {
  ...
  const statements = candidates.results.map((vault) =>
    env.DB
      ...
      .bind(crypto.randomUUID().replaceAll("-", ""), vault.vault_id, vault.vault_id, now, now, now),
  );
  if (statements.length > 0) await env.DB.batch(statements);
}
...
export async function maintenanceQueueMessage(env: Env, message: { job_id: string }): Promise<void> {
