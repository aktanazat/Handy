...
describe("allowlisted observability", () => {
  it("never emits sensitive request or encrypted-content canaries", () => {
    ...
    logRequest(
      env,
      { requestId: "request-canary", route: "object_manifest", startedAt: Date.now() },
      422,
    ...
    expect(Object.keys(JSON.parse(events[0] ?? "{}"))).toEqual(
      expect.arrayContaining(["request_id", "route", "status", "latency_bucket", "worker_version", "dependency", "error"]),
    );
    expect(Object.keys(JSON.parse(events[1] ?? "{}"))).toEqual(
      expect.arrayContaining(["request_id", "route", "status", "latency_bucket", "worker_version", "maintenance_class", "error"]),
    );
    } finally {
      log.mockRestore();
    }
