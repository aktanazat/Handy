; Custom NSIS template for Sona with portable mode support.
...
Function PageLeaveInstallType
  ...
  ${If} $0 = ${BST_CHECKED}
    ...
    ; --- PORTABLE MODE --- Switch the portable default to Desktop\${PRODUCTNAME}.
...
Function .onInit
  ...
  ; Preserve portable installs that use the Sona marker, the legacy Handy
  ; marker, or the legacy empty marker. Require Data/ for the empty-marker
  ; case so stale Scoop side-effect files do not opt an update into portable.
  ${If} $PortableMode <> 1
  ${AndIf} $UpdateMode = 1
...
Section Install
  ...
  ${If} $PortableMode = 1
    ...
    FileWrite $0 "Sona Portable Mode"
    FileClose $0
