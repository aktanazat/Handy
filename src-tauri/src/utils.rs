...
fn native_windows_machine() -> Option<u16> {
    ...
    // Resolve IsWow64Process2 dynamically so merely starting Sona never raises
    ...
    // SAFETY: kernel32.dll is a trusted system DLL, and the handle stays alive while its symbol is called.
    let kernel32 = unsafe { Library::new("kernel32.dll").ok()? };
    // SAFETY: the requested export has the documented IsWow64Process2 ABI and remains tied to kernel32.
    let is_wow64_process2: Symbol<IsWow64Process2> =
        unsafe { kernel32.get(b"IsWow64Process2\0").ok()? };
    let mut process_machine = 0u16;
    let mut native_machine = 0u16;
    // SAFETY: the resolved function has the declared ABI, and both output pointers refer to initialized writable u16 storage.
    unsafe {
        is_wow64_process2(
            GetCurrentProcess(),
            &mut process_machine,
...
pub fn is_gnome() -> bool {
    std::env::var("XDG_CURRENT_DESKTOP")
        .map(|value| value.to_uppercase().contains("GNOME"))
        .unwrap_or(false)
}
...
mod tests {
    ...
    #[cfg(all(target_os = "windows", target_arch = "x86_64"))]
    #[test]
    fn kernel32_lookup_distinguishes_present_and_absent_symbols() {
        use libloading::Library;
        use windows::Win32::Foundation::HANDLE;

        type GetCurrentProcess = unsafe extern "system" fn() -> HANDLE;

        // SAFETY: kernel32.dll is a trusted system DLL for the lifetime of this test.
        let kernel32 = unsafe { Library::new("kernel32.dll") }.expect("kernel32 must load");
        // SAFETY: GetCurrentProcess has the exact declared zero-argument system ABI.
        let present = unsafe { kernel32.get::<GetCurrentProcess>(b"GetCurrentProcess\0") };
        assert!(present.is_ok());
        // SAFETY: the deliberately nonexistent export cannot create a callable function pointer.
        let absent = unsafe { kernel32.get::<GetCurrentProcess>(b"SonaNoSuchSymbol\0") };
