...
/// hide below only unmaps the window when no show happened after it was
/// scheduled, so a hide left over from a finished transcription cannot take
/// down the overlay of a session that started in the meantime, such as a press
/// the coordinator remembered while the pipeline was busy and started the
