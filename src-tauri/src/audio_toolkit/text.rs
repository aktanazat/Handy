...
///   owner of spoken punctuation (see [`spoken_punctuation_at`], which already
...
///   turned the per-mode `literal_punctuation` choice off.
...
fn truncate_trailing(output: &mut String, marks: &[char]) {
    let keep = output
        .trim_end_matches(|character: char| marks.contains(&character))
        .len();
...
fn delete_last_word(output: &mut String) {
    truncate_trailing(output, HORIZONTAL_WHITESPACE);
    truncate_trailing(output, SEGMENT_BOUNDARY_MARKS);
    let keep = end_of_last_mark(output, &[' ', '\t', '\n', '\r']);
    output.truncate(keep);
    truncate_trailing(output, HORIZONTAL_WHITESPACE);
}
